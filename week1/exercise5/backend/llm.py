...
def describe(error: Exception) -> str:
    ...
    if isinstance(error, APITimeoutError):
        return (
            ...
            "A small model on CPU can be this slow — try a shorter question, "
            "turn streaming on, or point LLM_BASE_URL at a faster endpoint."
        )
...
async def complete(payload: list[dict], model: str) -> tuple[str, dict]:
...
async def _open_stream(payload: list[dict], model: str):
...
async def stream(payload: list[dict], model: str) -> AsyncIterator[dict]:
    ...
    try:
        upstream = await _open_stream(payload, model)
